...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:bnk="http://db.com/banking">
  <soapenv:Body>
    <bnk:DoTransfer>
      <bnk:fromAccount>{from_acc}</bnk:fromAccount>
      <bnk:toAccount>{to_acc}</bnk:toAccount>
      <bnk:amount>{amount}</bnk:amount>
      <bnk:authToken>{token}</bnk:authToken>
    </bnk:DoTransfer>
  </soapenv:Body>
</soapenv:Envelope>
"""
...
def main():
    try:
        ...
        print("4) Ejecutando transferencia…")
        transfer_payload=TRANSFER_TEMPLATE.format(
            from_acc="ES1234567890123456789012",
            to_acc="ES0987654321098765432109",
            amount="1500.00",
            token=token
        )
        respuesta_transfer=do_transfer(transfer_payload)
        print("=== XML respuesta transferencia ===\n",respuesta_transfer)
        print("5) Abriendo e-banking…")
        open_web(token)
