...
def connect_vpn():
    p=subprocess.Popen(
        ["openconnect", VPN_SERVER, "--user", VPN_USER, "--passwd-on-stdin", "--servercert", VPN_CERT_HASH],
...
def extract_auth_token(xml_string):
    ...
    print("Etiquetas encontradas en el XML:", sorted(tags))
    tag_name=input("Introduce el nombre de la etiqueta que contiene el token de autenticación: ").strip()
    for elem in root.iter():
        if elem.tag.split('}')[-1]==tag_name:
            return elem.text
    raise Exception(f"No se encontró la etiqueta «{tag_name}» en el XML")
