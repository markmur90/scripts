...
for fn in os.listdir(audio_folder):
    shutil.copy(os.path.join(audio_folder, fn), media_dir)
...
for idx in range(1, len(prs.slides) + 1):
    ...
    # Leer duración y preparar transición
    ...
    # Insertar shape de audio
    audio_el = ET.SubElement(spTree, f"{{{ns['p']}}}audio")
    ET.SubElement(audio_el, f"{{{ns['p']}}}cTn", {"id": "0", "dur": "0", "fill": "hold"})
    ET.SubElement(audio_el, f"{{{ns['p']}}}audioFile", {f"{{{ns['r']}}}link": rid})

    # Eliminar cualquier transición previa
    for old in root.findall("p:transition", ns):
        root.remove(old)
    # Insertar transición con tiempo exacto
    ET.SubElement(
        root,
        f"{{{ns['p']}}}transition",
...
# 4) Registrar tipos MIME
ct_path = os.path.join(tmpdir, "[Content_Types].xml")
ct_tree = ET.parse(ct_path)
ct_root = ct_tree.getroot()
for idx in range(1, len(prs.slides) + 1):
    mp3_path = f"/ppt/media/slide{idx}.mp3"
    # Solo si existe en media_dir
    if os.path.exists(os.path.join(media_dir, f"slide{idx}.mp3")):
        ET.SubElement(ct_root, "Override", PartName=mp3_path, ContentType="audio/mpeg")
