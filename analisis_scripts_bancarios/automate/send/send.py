...
import requests
from data import API_URL, DATA  # Importar DATA desde data.py
from utils import generate_end_to_end_identification, correlation_id, check_required_headers, generate_uuid  # Importar funciones necesarias
from config import CERT_PATH  # Importar CERT_PATH
from constants import paymentId  # Importar paymentId

# Configuración de logging
logger = logging.getLogger(__name__)
error_logger = logging.getLogger("error_logger")

def get_otp(paymentId=None):
    if paymentId is None:
        from constants import paymentId
    correlation_id_value = correlation_id(paymentId)  # Usar la función importada
    headers = {
