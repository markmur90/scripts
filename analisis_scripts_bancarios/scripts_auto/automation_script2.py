...
from collect_dic import escribir_diccionario_en_archivo
...
def generar_ips(rangos_ip: List[str]) -> List[str]:
    ...
    for rango in rangos_ip:
        if "-" in rango:
            ...
            for ip in red.hosts():
                if int(ip_address(ip)) > int(ip_address(fin)):
                    break
                ips.append(str(ip))
        else:
            ips.append(rango)
...
# Agregar la ruta del directorio diccionarios al sys.path
sys.path.append(os.path.join(BASE_DIR, "../diccionarios"))

# Importar la función para combinar archivos en diccionario
from collect_dic import combinar_archivos_en_diccionario
...
def main() -> None:
    ...
    # Generar diccionarios combinados
    ruta_diccionarios = os.path.join(BASE_DIR, "diccionarios")
    diccionarios_combinados = combinar_archivos_en_diccionario(ruta_diccionarios)
    logger.info(f"Diccionarios combinados generados: {diccionarios_combinados}")
    ...
    usuarios = leer_diccionario(os.path.join(BASE_DIR, "../diccionarios", config_base["archivo_usuarios"]))  # Asegúrate de que la ruta sea correcta
    contrasenas = leer_diccionario(os.path.join(BASE_DIR, "../diccionarios", config_base["archivo_contrasenas"]))  # Asegúrate de que la ruta sea correcta
